...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		int population_size = (int) Math.random()*100;
			...
			for(String buildingFile: buildingFiles) {
				...
				try {
					...
					do {
						...
						ArrayList<Genome> b_w = runSimulation(NLworkspace, buildingPath, wkspc, generations.get(generations.size()-1), generations.size()-1, population_size);
	...
	public static ArrayList<Genome> reproduce(ArrayList<Genome> p_subjects, ArrayList<Gen> ctx, int g_size) {
		...
		//by adding up both fitness values and then adding like gen1.x*(gen1_fit/added_fit)+gen2.x
		...
		for (int i = 0; i < p_subjects.size(); i++) {
			...
			//now the mating
			int SOF = father.getFitness()+mother.getFitness();//SOF stands for 'sumOfFitness'.
